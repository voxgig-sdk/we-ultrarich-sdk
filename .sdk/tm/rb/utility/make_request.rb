...
module WeUltrarichUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = WeUltrarichResponse.new({})
    ...
    if fetch_err
      ...
      response = WeUltrarichResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = WeUltrarichResponse.new(fetched)
    else
