...
module WeUltrarichUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = WeUltrarichContext.new({}, nil)
    end
    op = ctx.op || WeUltrarichOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || WeUltrarichResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(WeUltrarichError) ? err.msg : err.to_s
    ...
    sdk_err = WeUltrarichError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(WeUltrarichError)
