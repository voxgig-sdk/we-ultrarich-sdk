...
// WeUltrarich SDK utility: make_error

require_once __DIR__ . '/../core/Operation.php';
require_once __DIR__ . '/../core/Result.php';
require_once __DIR__ . '/../core/Error.php';

class WeUltrarichMakeError
{
    public static function call(?WeUltrarichContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new WeUltrarichContext([], null);
        }
        $op = $ctx->op ?? new WeUltrarichOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new WeUltrarichResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof WeUltrarichError) ? $err->msg : (string)$err;
        $msg = "WeUltrarichSDK: {$opname}: {$errmsg}";
        ...
        $sdk_err = new WeUltrarichError('', $msg, $ctx);
        ...
        if ($err instanceof WeUltrarichError) {
