...
// WeUltrarich SDK WealthExpression entity
...
class WealthExpressionEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new WealthExpressionEntity($this->_client, $opts);
    }

    /**
     * @param WealthExpression|array $args WealthExpression data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = WeUltrarichHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return WealthExpression|array The current WealthExpression data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of WealthExpression fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = WeUltrarichHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of WealthExpression fields).
     */
    ...
    /**
     * Load a single WealthExpression.
     *
     * @param WealthExpressionLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed WealthExpressionLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return WealthExpression|array The loaded WealthExpression as an assoc-array at the
     *   SDK boundary; throws WeUltrarichError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = WeUltrarichHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
