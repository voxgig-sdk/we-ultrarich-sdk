...
class FtCtrl extends WeUltrarichControl
{
...
class FtClient
{
    ...
    private WeUltrarichUtility $utility;

    public function __construct(WeUltrarichUtility $utility, array $options)
    {
    ...
    public function get_utility(): WeUltrarichUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (WeUltrarichContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public WeUltrarichContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (WeUltrarichContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = WeUltrarichConfig::make_feature($name);
    ...
    private static function buildUrl(WeUltrarichSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof WeUltrarichError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new WeUltrarichResponse($fetched);
        }
    ...
    private function fail(WeUltrarichContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (WeUltrarichContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
