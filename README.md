...
We > Ultrarich API client, generated from the OpenAPI spec.

How much is billions or trillions of dollars?

Learn more about We > Ultrarich API at [api.wegtultrarich.org/README.md](https://api.wegtultrarich.org/README.md).

This is an unofficial SDK for the We > Ultrarich public API, generated by Voxgig with [`@voxgig/sdkgen`](https://github.com/voxgig/sdkgen). It is not affiliated with, endorsed by, or sponsored by the upstream API provider.
...
This SDK exposes the API as a small set of **semantic entities** — Comparison, Discovery and WealthExpression — that you
...
```go
client := sdk.Test()
result, err := client.Comparison(nil).Load(
    nil, nil,
)
```
...
// Load comparison data (returns a Comparison)
...
```bash
cd go-mcp && go build -o we-ultrarich-mcp .
```
...
```json
{
  "mcpServers": {
    "we-ultrarich": {
      "command": "/abs/path/to/we-ultrarich-mcp"
    }
  }
}
```
...
| --- | --- | --- |
| **Comparison** | The Comparison entity (load). | `/comparison` |
| **Discovery** | The Discovery entity (list). | `/expressions` |
| **WealthExpression** | The WealthExpression entity (load). | `/growthOfCompoundInterest` |
...
# Load a specific comparison (returns the record, raises on error)
...
```go
import sdk "github.com/voxgig-sdk/we-ultrarich-sdk/go"
...
Generated from the We > Ultrarich API OpenAPI spec by [@voxgig/sdkgen](https://github.com/voxgig/sdkgen).
