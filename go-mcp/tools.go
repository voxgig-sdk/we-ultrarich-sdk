...
type Args struct {
	Entity string         `json:"entity" jsonschema:"comparison | discovery | wealth_expression"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.WeUltrarichSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "we-ultrarich_list",
		Description: "List records from WeUltrarich. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "we-ultrarich_load",
		Description: "Load a single record from WeUltrarich. " +
...
func runOp(client *sdk.WeUltrarichSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.WeUltrarichSDK, name string) (sdk.WeUltrarichEntity, error) {
	switch strings.ToLower(name) {
	case "comparison":
		return client.Comparison(nil), nil
	case "discovery":
		return client.Discovery(nil), nil
	case "wealth_expression":
		return client.WealthExpression(nil), nil
