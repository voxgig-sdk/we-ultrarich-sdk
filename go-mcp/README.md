...
[MCP](https://modelcontextprotocol.io) server exposing the WeUltrarich SDK as
two agent tools — `we-ultrarich_list` and `we-ultrarich_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/we-ultrarich-mcp)
make build

# 2. Provide credentials via the environment
export WE_ULTRARICH_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user we-ultrarich \
  -- /absolute/path/to/we-ultrarich-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./we-ultrarich-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// we-ultrarich_list: first page of records
{ "entity": "discovery" }
{ "entity": "discovery", "query": { } }

// we-ultrarich_load: one record by id
{ "entity": "comparison", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/we-ultrarich-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user we-ultrarich \
     -- "$PWD"/dist/*/we-ultrarich-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `we-ultrarich_list` and `we-ultrarich_load` tools now appear
   in new sessions. Ask the agent to *"list discovery using we-ultrarich"*
   and it calls `we-ultrarich_list` with `{"entity":"discovery"}`.
...
```sh
export WE_ULTRARICH_APIKEY=sk_live_xxx            # API key
export WE_ULTRARICH_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `we-ultrarich_list` tool
...
### Call the `we-ultrarich_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `we-ultrarich_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `we-ultrarich_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `WE_ULTRARICH_APIKEY` | API key sent with every request. |
| `WE_ULTRARICH_BASE` | Optional override of the API base URL. |

### Entities

The 3 entities valid as the `entity` argument:

comparison | discovery | wealth_expression

### Smoke test via HTTP (raw JSON-RPC)

```sh
./we-ultrarich-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"we-ultrarich_load","arguments":{"entity":"comparison","query":{"id":1}}}}'
```
