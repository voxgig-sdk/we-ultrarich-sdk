...
local ComparisonEntity = {}
ComparisonEntity.__index = ComparisonEntity


function ComparisonEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ComparisonEntity)
...
function ComparisonEntity:get_name()
  return self._name
end


function ComparisonEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ComparisonEntity.new(self._client, opts)
end
...
function ComparisonEntity:mark_deleted()
  self._deleted = true
end


function ComparisonEntity:deleted()
  return true == self._deleted
end


function ComparisonEntity:data_set(args)
  if args ~= nil then
...
function ComparisonEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ComparisonEntity:match_set(args)
  if args ~= nil then
...
function ComparisonEntity:match_get()
...
function ComparisonEntity:stream(action, args, callopts)
...
---@param reqmatch ComparisonLoadMatch
---@param ctrl? table
---@return Comparison
---@return string? err
function ComparisonEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function ComparisonEntity:_run_op(ctx, post_done)
  local utility = self._utility
