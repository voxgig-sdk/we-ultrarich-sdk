...
local DiscoveryEntity = {}
DiscoveryEntity.__index = DiscoveryEntity


function DiscoveryEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DiscoveryEntity)
  self._name = "discovery"
  self._client = client
...
function DiscoveryEntity:get_name()
  return self._name
end


function DiscoveryEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DiscoveryEntity.new(self._client, opts)
end
...
function DiscoveryEntity:mark_deleted()
  self._deleted = true
end


function DiscoveryEntity:deleted()
  return true == self._deleted
end


function DiscoveryEntity:data_set(args)
  if args ~= nil then
...
function DiscoveryEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DiscoveryEntity:match_set(args)
  if args ~= nil then
...
function DiscoveryEntity:match_get()
...
function DiscoveryEntity:stream(action, args, callopts)
...
---@param reqmatch DiscoveryListMatch
---@param ctrl? table
---@return Discovery[]
---@return string? err
function DiscoveryEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function DiscoveryEntity:_run_op(ctx, post_done)
  local utility = self._utility
