...
local WealthExpressionEntity = {}
WealthExpressionEntity.__index = WealthExpressionEntity


function WealthExpressionEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, WealthExpressionEntity)
  self._name = "wealth_expression"
  self._client = client
...
function WealthExpressionEntity:get_name()
  return self._name
end


function WealthExpressionEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return WealthExpressionEntity.new(self._client, opts)
end
...
function WealthExpressionEntity:mark_deleted()
  self._deleted = true
end


function WealthExpressionEntity:deleted()
  return true == self._deleted
end


function WealthExpressionEntity:data_set(args)
  if args ~= nil then
...
function WealthExpressionEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function WealthExpressionEntity:match_set(args)
  if args ~= nil then
...
function WealthExpressionEntity:match_get()
...
function WealthExpressionEntity:stream(action, args, callopts)
...
---@param reqmatch WealthExpressionLoadMatch
---@param ctrl? table
---@return WealthExpression
---@return string? err
function WealthExpressionEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function WealthExpressionEntity:_run_op(ctx, post_done)
  local utility = self._utility
