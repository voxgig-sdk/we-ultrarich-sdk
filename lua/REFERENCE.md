# WeUltrarich Lua SDK Reference

Complete API reference for the WeUltrarich Lua SDK.
...
```lua
local comparison = client:Comparison(nil)
```
...
```lua
local result, err = client:Comparison():load({ expression = "expression", wealth_their = "wealth_their", wealth_your = "wealth_your" })
```
...
```lua
local discovery = client:Discovery(nil)
```
...
```lua
local results, err = client:Discovery():list()
```
...
```lua
local wealth_expression = client:WealthExpression(nil)
```
...
```lua
local result, err = client:WealthExpression():load({ wealth = "wealth" })
```
