...
const TEST_SEED = { "entity": { "comparison": { "test01": { "id": "test01" } }, "discovery": { "test01": { "id": "test01" } }, "wealth_expression": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+WeUltrarichSDK\b/.test(code) ||
        /\bWeUltrarichSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+WeUltrarichSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/WeUltrarichSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
