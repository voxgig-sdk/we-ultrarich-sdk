...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.WEULTRARICH_TEST_LIVE ||
    'TRUE' === process.env.WEULTRARICH_TEST_OVERRIDE
  ) {
  ...
  m.WEULTRARICH_TEST_EXPLAIN = process.env.WEULTRARICH_TEST_EXPLAIN || m.WEULTRARICH_TEST_EXPLAIN
