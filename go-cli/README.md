...
boru-driven command-line client **and** interactive REPL for the WeUltrarich
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/we-ultrarich-cli)
make build

# 2. See usage (words, entities, env vars)
./we-ultrarich-cli --help

# 3. Provide credentials once, via the environment
export WE_ULTRARICH_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./we-ultrarich-cli load 1 comparison            # {id:1} shorthand
./we-ultrarich-cli load '{id:1}' comparison       # explicit match map

# 5. Override the API base URL for a single call
WE_ULTRARICH_BASE=https://api.example.com ./we-ultrarich-cli load 1 comparison

# 6. No arguments -> interactive REPL
./we-ultrarich-cli
we-ultrarich> load 1 comparison
we-ultrarich> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/we-ultrarich-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/we-ultrarich-cli load 1 comparison
   ```
...
```sh
./we-ultrarich-cli load 1 comparison          # scalar shorthand for {id:1}
./we-ultrarich-cli load '{id:1}' comparison     # explicit match map
```
...
./we-ultrarich-cli load 1 comparison
```
...
Run with no arguments to open a REPL (prompt `we-ultrarich>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `comparison`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/we-ultrarich-cli`. |
...
(`./we-ultrarich-cli <expr>`) and interactively (the REPL), and expressions compose the
