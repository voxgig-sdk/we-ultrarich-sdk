// Command WeUltrarich-cli is a CLI + REPL for the WeUltrarich SDK,
...
const prompt = "we-ultrarich"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "comparison discovery wealth_expression"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      WE_ULTRARICH_APIKEY (api key), WE_ULTRARICH_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: WE_ULTRARICH_APIKEY carries the API key and
	// WE_ULTRARICH_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("WE_ULTRARICH_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("WE_ULTRARICH_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewWeUltrarichSDK(opts)
