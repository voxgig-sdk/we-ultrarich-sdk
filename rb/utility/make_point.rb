...
module WeUltrarichUtilities
  MakePoint = ->(ctx) {
    if ctx.out["point"]
      ...
      return nil, preset if preset.is_a?(WeUltrarichError)
      ctx.point = preset
    ...
    if op.points.length == 1
      ...
      matched = false
      op.points.each do |p|
        select_def = WeUltrarichHelpers.to_map(VoxgigStruct.getprop(p, "select"))
        found = true
      ...
      if reqselector
        req_action = VoxgigStruct.getprop(reqselector, "$action")
        if req_action && point
          point_select = WeUltrarichHelpers.to_map(VoxgigStruct.getprop(point, "select"))
