...
class WealthExpressionEntity
  ...
  def make
    opts = @_entopts.dup
    WealthExpressionEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = WeUltrarichHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [WealthExpression, Hash] the current WealthExpression data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = WeUltrarichHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of WealthExpression fields)
  def match_get
  ...
  #
  # @param reqmatch [WealthExpressionLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.WealthExpression.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [WealthExpression, Hash] the loaded WealthExpression; raises WeUltrarichError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = WeUltrarichHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
