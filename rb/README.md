# WeUltrarich Ruby SDK



The Ruby SDK for the WeUltrarich API — an entity-oriented client using idiomatic Ruby conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.Comparison` — with named operations (`list`/`load`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
- Releases: [https://github.com/voxgig-sdk/we-ultrarich-sdk/releases](https://github.com/voxgig-sdk/we-ultrarich-sdk/releases)
...
```ruby
client = WeUltrarichSDK.test(testopts, sdkopts)
```
...
raise a `WeUltrarichError` (a `StandardError` subclass), so wrap
...
Create an instance: `comparison = client.Comparison`
...
| `ratio` | `Hash` | The ratio between the two wealths (wealthTheirs divided by wealthYours; values below 1 mean wealthYours is the larger). |
| `resultTheirs` | `Hash` | The expression's result for wealthTheirs. |
| `resultYours` | `Hash` | The expression's result for wealthYours. |
...
Create an instance: `discovery = client.Discovery`
...
```ruby
# list returns an Array of Discovery records (raises on error).
discoverys = client.Discovery.list
```


### WealthExpression

Create an instance: `wealth_expression = client.WealthExpression`
...
```
rb/
├── WeUltrarich_sdk.rb       -- Main SDK module
...
The main module (`WeUltrarich_sdk`) exports the SDK class
