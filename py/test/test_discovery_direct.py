...
class TestDiscoveryDirect:

    def test_should_direct_list_discovery(self):
        setup = _discovery_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-discovery", "live" if setup["live"] else "unit")
        if _skip:
...
def _discovery_direct_setup(mockres):
