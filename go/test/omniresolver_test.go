...
import (
	...
	omni "github.com/voxgig-sdk/we-ultrarich-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.WeUltrarichError); is {
				out := map[string]any{
					"name":    "WeUltrarichError",
...
func omniCtx(arg any, client *sdk.WeUltrarichSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.WeUltrarichSDK); is {
			client = live
