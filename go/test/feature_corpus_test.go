...
//
// NewWeUltrarichSDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.WeUltrarichSDK {
	res, _ := kase["res"].([]any)
	opts := map[string]any{
	...
	return sdk.NewWeUltrarichSDK(opts)
}
...
func fcCandidates(client *sdk.WeUltrarichSDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.WeUltrarichSDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.WeUltrarichSDK, name string) any {
			...
			for _, raw := range cases {
				...
				for _, rawstep := range steps {
					...
					if code, ok := wanterr.(string); ok {
						...
						// the word.
						got := ""
						var sdkerr *sdk.WeUltrarichError
