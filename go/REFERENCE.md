# WeUltrarich Golang SDK Reference

Complete API reference for the WeUltrarich Golang SDK.
...
#### `TestSDK(testopts, sdkopts map[string]any) *WeUltrarichSDK`
...
#### `Comparison(data map[string]any) WeUltrarichEntity`

Create a new `Comparison` entity instance. Pass `nil` for no initial data.

#### `Discovery(data map[string]any) WeUltrarichEntity`

Create a new `Discovery` entity instance. Pass `nil` for no initial data.

#### `WealthExpression(data map[string]any) WeUltrarichEntity`
...
fmt.Println(comparison.GetName()) // "comparison"
```
...
```go
result, err := client.Comparison(nil).Load(map[string]any{"expression": "expression", "wealth_their": "wealth_their", "wealth_your": "wealth_your"}, nil)
if err != nil {
    panic(err)
}
fmt.Println(result)
...
```go
discovery := client.Discovery(nil)
fmt.Println(discovery.GetName()) // "discovery"
```
...
```go
results, err := client.Discovery(nil).List(nil, nil)
if err != nil {
    panic(err)
}
fmt.Println(results)
...
```go
wealthExpression := client.WealthExpression(nil)
fmt.Println(wealthExpression.GetName()) // "wealth_expression"
```
...
```go
result, err := client.WealthExpression(nil).Load(map[string]any{"wealth": "wealth"}, nil)
if err != nil {
    panic(err)
}
fmt.Println(result)
